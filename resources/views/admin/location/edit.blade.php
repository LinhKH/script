                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                                        ...
                                        <select class="form-control form-control-sm" name="category" id="categorySelect"
                                            ...
                                            @if (!empty($categories))
                                                @foreach ($categories as $types)
                                                    <option value="{{ $types->id }}" {{ $types->id === $location->category_id ? 'selected' : '' }}>{{ $types->title }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    </div>
                            ...
                            <div class="col-2 text-center">
                                @if($location->image != '')
                                    <img id="image" src="{{asset('location/'.$location->image)}}" alt="" width="100px" height="80px">
                                @else
                                    <img id="image" src="{{asset('location/default.jpg')}}" alt="" width="100px" height="80px">
                                @endif
                            </div>      
                        </div>
                    </div>
